...
class OCRPipeline:
    def __init__(self, raw_dir: str = "data/raw", interim_dir: str = "data/interim", ocr_dir: str = "data/ocr", dpi: int = 300):
    ...
    def process_document(self, case_id: str, document_id: str, source_path: str):
        ...
        for page_index, page_img in pages:
            ...
            result = self.service.build_result(case_id, document_id, page_index, str(source_path), bgr.shape, quality, raw)
        ...
        manifest.write_text(json.dumps({"case_id": case_id, "document_id": document_id, "pages": results}, indent=2), encoding="utf-8")
        return {"status": "OCR_DONE", "manifest": str(manifest), "pages": results}