...
# If Ollama fails, still require review unless rule confidence is very strong
OLLAMA_FAILURE_REVIEW_THRESHOLD = 0.80
...
class ConfidenceScorer:
    ...
    def merge(
        ...
        If ollama_result is None (skipped or failed before response), uses rule engine only.
        """
        rule_doc = rule_result.get("doc_type", "unknown")
        rule_conf = rule_result.get("confidence", 0.0)

        # Case 1: Ollama not used at all
        ...
        # Case 2: Ollama errored or timed out — fall back to rule engine, be conservative
        if ollama_error:
            final_doc = rule_doc
            final_conf = rule_conf
            method = "rule_engine"
            ...
            needs_review_due_to_ollama_failure = final_conf < OLLAMA_FAILURE_REVIEW_THRESHOLD
            ...
            return ClassificationResult(
                ...
                needs_human_review=(
                    final_doc == "unknown"
                    or final_conf < HUMAN_REVIEW_THRESHOLD
                    or needs_review_due_to_ollama_failure
                ),
                all_scores=rule_result.get("all_scores", {}),
            )
        ...
        if rule_doc == ollama_doc and rule_doc != "unknown":
            ...
            final_doc = rule_doc if rule_conf >= ollama_conf else ollama_doc
